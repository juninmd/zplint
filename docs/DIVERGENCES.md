...
| 2.6 | **`#pragma ctrlchar` é posicional, tratamos como valor único** | O amxxpc intercala preprocessamento e lexing, então a troca vale **a partir daquele ponto**. O `zpc` preprocessa tudo primeiro e depois escaneia o texto expandido com **um** ctrlchar — o valor final. Um plugin que faz `#pragma ctrlchar '\'` depois dos `#include` faz o scanner reler os headers (escritos para `^`) com `\`. **Bug conhecido, não uma escolha.** Afeta 1 dos 74 plugins oficiais (`strbreak.sma` → erro 027 em `string_stocks.inc:178`, onde `'\'` deixa de ser o caractere barra-invertida). Correção correta: o preprocessador registrar `(posição, ctrlchar)` e o scanner aplicar a troca ao cruzar cada ponto — mudança de design, não remendo. |
